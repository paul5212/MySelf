    ...
    for entry in entries:
        ...
        try:
            ...
            results.append({
                ...
                'center': f"({cx},{cy})",
                'status': status,
                'variance': var if var == 'Error' else str(var),
                'median': f"{med:.1f}" if med != 'Error' else 'Error',
...
if os.path.exists(parse_txt_path):
    try:
        ...
        for line_idx, line in enumerate(lines, start=1):
            ...
            parsed_entries.append({
                'line_num': line_idx,
                'entry_id': f"{len(parsed_entries)+1:03d}",  # 從 001 開始編號
                'ip': ip,
                'slice_num': slice_num,
                'center_x': center_x,
                'center_y': center_y,
        ...
        print_status(f"有效解析筆數：{len(parsed_entries)}")
    ...
    parsed_entries.sort(key=lambda e: (
        int(e['ip'][2:]),      # IP 數字
        e['slice_num'],        # Slice 小→大
        e['center_y']          # Y 小→大（上→下）
    ))
