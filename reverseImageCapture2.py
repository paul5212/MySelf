...
def process_entries(entries, folder_name, folder_path_base):
    ...
    for entry in entries:
        ...
        # 產生唯一檔名：001_IP08_Origin000006_y3415.tif
        base_name, ext = os.path.splitext(orig_filename)
        y_part = f"_y{cy}"
        output_filename = f"{entry_id}_{base_name}{y_part}{ext}"
        ...
        try:
            ...
            results.append({
                ...
                'center': f"({cx},{cy})",
                'status': status,
                'variance': f"{var:.1f}" if isinstance(var, (int, float)) else var,
...
if os.path.exists(parse_txt_path):
    ...
    # 排序 parsed_entries
    parsed_entries.sort(key=lambda e: (
...
with open(report_path, 'w', encoding='utf-8') as f:
    ...
    f.write(f"{'#':<4} {'IP':<6} {'Slice':<6} {'Center':<15} {'Status':<8} {'Variance':<10} {'Median':<8} {'Copy':<12} {'Output File':<40}\n")
    f.write("-" * 120 + "\n")

    blur_count = 0
    for r in all_results:  # 已經排序
        status_str = {0: 'Normal', 1: 'Blur', -1: 'Error'}.get(r['status'], 'Unknown')
        if r['status'] == 1:
            blur_count += 1
        f.write(f"{r['entry_id']:<4} {r['ip']:<6} {r['slice']:<6} {r['center']:<15} {status_str:<8} {r['variance']:<10} {r['median']:<8} {r['copy']:<12} {r['output_file']:<40}\n")
