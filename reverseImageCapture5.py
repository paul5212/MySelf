...
def compute_blur_and_draw_roi(image_path, center_x, center_y, ip, slice_num):
    try:
        ...
        # 計算理想的 ROI 左上角（不 clip）
        ideal_left = cx - roi_width // 2
        ideal_top  = cy - roi_height // 2
        ...
        # 截取彩色 ROI（畫框前，確保無紅框）
        roi_img = img_array[ry:ry + rh, rx:rx + rw].copy()

        # 計算需要補的邊距（補到固定 1200×800）
        pad_left   = max(0, -ideal_left)                  # 左邊超出
        pad_top    = max(0, -ideal_top)                   # 上邊超出
        pad_right  = max(0, (ideal_left + roi_width) - w)   # 右邊超出
        pad_bottom = max(0, (ideal_top  + roi_height) - h)  # 下邊超出

        # 使用 copyMakeBorder 補黑邊，保證輸出永遠是 1200×800
        roi_img_padded = cv2.copyMakeBorder(
            roi_img,
            pad_top,
            pad_bottom,
            pad_left,
        ...
        # 確認大小（debug 可選）
        # assert roi_img_padded.shape[:2] == (roi_height, roi_width), "ROI 大小不符！"
        ...
        if save_roi:
            ...
            cv2.imwrite(roi_path, roi_img_padded)
