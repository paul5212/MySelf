...
if mode == 'ALL':
    ...
    for sub in o_subfolders:
        # 解析名稱中的時間 (假設格式: prefix_YYYYMMDDHHMM_Src)
        try:
            parts = sub.split('_')
            if len(parts) != 3 or parts[2] != 'Src':
                raise ValueError("名稱格式不符合")
            time_str = parts[1]  # e.g., '202601201449'
            parsed_time = datetime.strptime(time_str, '%Y%m%d%H%M')
        except Exception as e:
            print_status(f"[O] 解析名稱時間失敗 ({sub}): {e}，跳過")
            continue
        
        full_path = os.path.join(o_base_path, sub)
        o_src_folders.append((sub, full_path, parsed_time))
        print_status(f"  [O] 發現資料夾: {sub} (解析時間: {parsed_time})")
    
    if not o_src_folders:
        print_status("[O] 無有效資料夾，無法繼續")
        sys.exit(1)
    
    # 找 O drive 解析時間最晚的資料夾
    latest_o = max(o_src_folders, key=lambda x: x[2])
    latest_folder_name = latest_o[0]
    print_status(f"\n=== O drive 計算出的最新資料夾 (基於名稱時間): {latest_folder_name} ===")
    ...
    try:
        with open(txt_path, 'w', encoding='utf-8') as f:
            f.write(f"最新資料夾 (基於 O drive 名稱時間): {latest_folder_name}\n")
            f.write(f"日期: {date}\n")
            f.write(f"處理時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"模糊閾值: {blur_threshold}\n")
            f.write("="*50 + "\n")
