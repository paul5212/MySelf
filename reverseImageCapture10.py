...
def process_entries(entries, folder_name, folder_path_base):
    ...
    # 新資料夾結構
    image_folder = os.path.join(output_dir, folder_name, 'Image')
    original_folder = os.path.join(image_folder, 'Original')
    roi_folder = os.path.join(image_folder, 'ROI')
    ...
    for entry in entries:
        ...
        # 處理大圖輸出（放在 Image/Original）
        if save_big_image:
            big_path = os.path.join(original_folder, big_filename)
            shutil.copy(orig_path, big_path)
            copy_status = f"Success ({'Box' if draw_roi_box else 'No Box'})"
            print_status(f"複製大圖到 Image/Original: {big_filename} ({'加框' if draw_roi_box else '原圖'})")
        ...
        if need_full_process:
            ...
            if save_roi and roi_fixed is not None:
                ...
                print_status(f"ROI 截圖儲存到 Image/ROI: {roi_filename}")
...
# =============================================================================
# 輸出報告（放在 folder_name 根目錄）
# =============================================================================
all_results.sort(key=lambda r: int(r['entry_id']))

root_folder = os.path.join(output_dir, folder_name)
os.makedirs(root_folder, exist_ok=True)
report_filename = f"{folder_name}_report.txt"
report_path = os.path.join(root_folder, report_filename)
...
with open(report_path, 'w', encoding='utf-8') as f:
    ...
    f.write(f"    Image/\n")
    f.write(f"      Original/   ← 大圖（原圖或加框圖）\n")
    f.write(f"      ROI/        ← ROI 小圖\n")
