...
# LATEST 模式專用: 指定要分析的資料夾索引 (0=最新, 1=第二新, ...)
latest_index = 0
...
def process_drive(drive, folder_name, status_dict, variance_dict, median_dict):
    ...
    for ip in target_ips:
        ...
        try:
            ...
            print_status(f"  [{drive}-{ip}] Center: ({center_x_actual},{center_y_actual}) | Variance: {lap_variance:.1f} | Median Gray: {median_gray:.1f} → {'Blur' if status else 'Normal'}")
...
if mode == 'ALL':
    ...
    # LATEST 模式: 先找 O 的所有 _Src 資料夾，按 mtime 降序排序，選擇指定索引的資料夾名稱
    o_base_path = f"O:\\Image\\{date}"
    print_status(f"\n[O] 掃描路徑 (用於計算資料夾列表): {o_base_path}")
    
    if not os.path.exists(o_base_path):
        print_status("[O] 路徑不存在，無法計算資料夾列表")
        sys.exit(1)
    ...
    if not o_subfolders:
        print_status("[O] 無 _Src 資料夾，無法計算列表")
        sys.exit(1)
    ...
    # 按 mtime 降序排序 (最新在前)
    o_src_folders.sort(key=lambda x: x[2], reverse=True)
    
    print_status("\nO drive 資料夾列表 (按時間降序):")
    for idx, (name, _, mtime) in enumerate(o_src_folders):
        print_status(f"  Index {idx}: {name} (mtime: {datetime.fromtimestamp(mtime)})")
    
    try:
        latest_folder_name = o_src_folders[latest_index][0]
        print_status(f"\n選擇 Index {latest_index} 的資料夾: {latest_folder_name}")
    except IndexError:
        print_status(f"錯誤: Index {latest_index} 超出範圍 (可用範圍: 0 ~ {len(o_src_folders)-1})")
        sys.exit(1)
    ...
    try:
        with open(txt_path, 'w', encoding='utf-8') as f:
            f.write(f"選擇資料夾 (Index {latest_index}, 基於 O drive): {latest_folder_name}\n")
            f.write(f"基準中心: ({base_center_x}, {base_center_y})\n")
            f.write(f"日期: {date}\n")
            f.write(f"處理時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"模糊閾值: {blur_threshold}\n")
